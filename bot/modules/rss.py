...
async def rssSub(client, message, pre_event):
    ...
    for item in items:
        ...
        if len(args) > 2:
            arg = item.split(' c: ', 1)
            cmd = re_split(' inf: | exf: | up: ', arg[1])[
                0].strip() if len(arg) > 1 else None
            arg = item.split(' inf: ', 1)
            inf = re_split(' c: | exf: | up: ', arg[1])[
                0].strip() if len(arg) > 1 else None
            arg = item.split(' exf: ', 1)
            exf = re_split(' c: | inf: | up: ', arg[1])[
                0].strip() if len(arg) > 1 else None
            arg = item.split(' up: ', 1)
            up = re_split(' c: | inf: | exf: ', arg[1])[
                0].strip() if len(arg) > 1 else None
            if inf is not None:
            ...
            async with rss_dict_lock:
                if rss_dict.get(user_id, False):
                    ...
                    rss_dict[user_id] = {title: {'link': feed_link, 'last_feed': last_link, 'last_title': last_title,
                                                 'inf': inf_lists, 'exf': exf_lists, 'paused': False, 'command': cmd, 'up_path': up, 'tag': tag}}
            LOGGER.info(
                f"Rss Feed Added: id: {user_id} - title: {title} - link: {feed_link} - c: {cmd} - inf: {inf} - exf: {exf} - up: {up}")
                ...
                while True:
                    ...
                    try:
                        ...
                        try:
                            ...
                            pub_date = None
                        try:
                            description = rss_d.entries[feed_count]['description']
                        except:
                            description = None
                        try:
                    ...
                    if command := data['command']:
                        up = f"up: {up_path}" if (
                            up_path := data['up_path']) else ''
                        feed_msg = f"/{command.replace('/', '')} {url} {up}\n<b>Tag :</b> {data['tag']} | <b>ID :</b> <code>{user}</code>"
                    else:
