...
async def _auto_cancel(msg, task_id):
    ...
    try:
        ...
        await editMessage(msg, 'Waktu habis! Tugas dibatalkan')
    except:
        pass
...
async def _ytdl(client, message, isZip=False, isLeech=False, sameDir={}):
    ...
    if qual:
        ...
        if 'entries' in result:
            ...
            bmsg = await sendMessage(message, '<b>Pilih kualitas Video Playlist :</b>', mbuttons)
        else:
            ...
            bmsg = await sendMessage(message, '<b>Pilih kualitas Video :</b>', mbuttons)
...
async def select_format(client, query):
    ...
    try:
        ...
        await editMessage(message, "Bukan tugas aktif!")
        return
    uid = task_info[1]
