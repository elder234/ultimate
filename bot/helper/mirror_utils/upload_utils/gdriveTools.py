...
class GoogleDriveHelper:
        ...
        try:
            if ospath.isfile(item_path):
                if item_path.lower().endswith(tuple(GLOBAL_EXTENSION_FILTER)):
                    raise Exception('Ekstensi file ini diblokir oleh bot!')
        ...
        for drive_name, dir_id, index_url in zip(DRIVES_NAMES, DRIVES_IDS, INDEX_URLS):
            ...
            if not response["files"]:
                if noMulti:
                    ...
                    continue
            if not Title:
                msg += f'<h4>Hasil pencarian kata kunci {fileName}</h4>'
                Title = True
            if drive_name:
    ...
    def count(self, link):
        try:
            ...
            return "ID Google Drive tidak ditemukan!", None, None, None, None
        msg = ""
        LOGGER.info(f"File ID: {file_id}")
        try:
    ...
    async def cancel_download(self):
        ...
        if self.__is_downloading:
            ...
            await self.__listener.onUploadError('Clone dibatalkan oleh User!')
