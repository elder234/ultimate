...
class TelegramDownloadHelper:
    ...
    async def __download(self, message, path):
        try:
            ...
            if self.__is_cancelled:
                await self.__onDownloadError('Unduhan dibatalkan oleh User!')
                return
    ...
    async def add_download(self, message, path, filename, session):
        ...
        if media is not None:
            ...
            if download:
                ...
                await self.__onDownloadError('File ini sudah ditambahkan di tugas unduh!')
        else:
            await self.__onDownloadError('Tidak ada dokumen/media di pesan yang dibalas!')
