...
class RcloneDownloadHelper:
    ...
    async def add_download(self, rc_path, config_path, path, name, from_queue=False):
        ...
        if all_limit or dl_limit:
            ...
            async with queue_dict_lock:
                ...
                if (all_limit and dl + up >= all_limit and (not dl_limit or dl >= dl_limit)) or (dl_limit and dl >= dl_limit):
                    added_to_queue = True
                    queued_dl[self.__listener.uid] = ['rcd', rc_path,
                                                      config_path, path, name, self.__listener]
            if added_to_queue:
        ...
        cmd = ['rclone', 'copy',
               f'--config={config_path}', '-P', rc_path, path, '--exclude', ext]
        ...
        if return_code == 0:
            ...
            await self.__listener.onDownloadError('Ada yang salah!')
